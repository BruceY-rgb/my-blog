...
description: >-
  1. 反转整个链表 1.1 迭代解法 迭代解法是一个非常简单的思路，新建一个空链表，原链表顺序访问，每个节点使用 头插法逐个插入到新链表中
  ，新链表就是一个反转之后的链表 方法本身不难，但是要注意一些问题： 1. 如果出现 then.next 这种操作，就要条件反射地想到，先判断 then
  是否为 null ，否则容易
published: true
legacyPath: 2026/08/14/leetcode/singlelinkreverse
sourcePath: leetcode/singlelinkreverse.md
---
...
![以2个节点为一组反转链表1](/my-blog/2026/08/14/leetcode/singlelinkreverse/image-1.png)
...
![以2个节点为一组反转链表2](/my-blog/2026/08/14/leetcode/singlelinkreverse/image-2.png)

发现了递归性质，就可以得到大致的算法流程：

1. 先反转以`head`开头的`k`个元素，这里可以复用前面实现的`reverseN`函数

![反转开头的k个元素](/my-blog/2026/08/14/leetcode/singlelinkreverse/image-3.png)

2. 将第`k + 1`元素作为`head`递归调用`reverseKGroup`

![递归调用](/my-blog/2026/08/14/leetcode/singlelinkreverse/image-4.png)

3. 将上述两个过程的结果连接起来

![连接两个链表](/my-blog/2026/08/14/leetcode/singlelinkreverse/image-5.png)
