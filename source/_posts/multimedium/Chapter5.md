...
对于具有频率下限f1和上限f2的限带信号，我们需要至少 **2(f2-f1)** 的采样率。
...
A. **变换**：将输入数据转换为更容易或更有效压缩的新表示
B. **量化**：引入信息丢失的主要步骤——使用有限的重构电平，少于原始信号
...
简单地说，就是传输差异——预测下一个样本等于当前样本；不发送样本本身，而是发送前一个与下一个之间的差异。
