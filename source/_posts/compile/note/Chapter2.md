...
```
IF LPAREN ID(i) RELOP(==) ID(j) RPAREN ID(z) ASSIGN NUM(0) SEMI ELSE ID(z) ASSIGN NUM(1) SEMI
```

**其他辅助任务**：词法分析器还会过滤注释、空白字符（空格、制表符、换行符）等。
...
| 概念 | 英文 | 描述 |
|------|------|------|
| **Token** | 词法记号/单词 | 语言的语法单位，如同英语中的名词、动词、形容词；编程语言中的关键字、标识符、运算符等 |
| **Lexeme** | 词素 | Token 的具体实例，即源程序中符合某个模式的字符序列 |
| **模式** | Pattern | 描述 Token 所属 Lexeme 的规则/定义 |
...
对于比较复杂的语言，为了构造简洁的正则式，可以先构造简单的正则式，再将这些正则式组合起来。
...
```
digit → 0 | 1 | ... | 9          # 可以简记为 [0-9]
number → digit digit*            # 或者：digit+（正闭包）
```
...
同一个正则表达式可以在不同的代数结构中求值，从而解决不同的问题！
...
> **注意**：如果转换函数没有给出对应于某个状态-输入对的信息，就把 ∅ 放入相应的表项中

### 3.3 有穷自动机接收的串

- 给定输入串 x，如果存在一个对应于串 x 的从初始状态到某个终止状态的转换序列，则称串 x 被该 FA **接收**
- 由一个有穷自动机 M 接收的所有串构成的集合，称为该 FA 接收（或定义）的语言，记作 **L(M)**
...
该 FA 接收的语言：L(M) = **{xabb | x ∈ {a, b}*}**（所有以 "abb" 结尾的串）
...
- **转换函数**：move(s, a) → P(S)（从状态 s 出发，读入 a 后可能到达多个状态）
- **可能存在 ε-moves**：不读入任何输入就能转移到其他状态
- **1976 年图灵奖**：Michael O. Rabin 和 Dana S. Scott 因对 NFA 理论贡献获奖

#### 确定性有穷自动机 (DFA)

- **转换函数**：move(s, a) → S（从状态 s 出发，读入 a 后到达**唯一确定**的状态）
- **没有 ε-moves**
...
**子集构造法**的核心理念：DFA 的每个状态对应 NFA 状态集合的一个子集。

#### 关键操作

| 操作 | 定义 |
|------|------|
| ε-closure(s) | 从 NFA 状态 s 出发，只通过 ε 转换能到达的所有状态 |
...
```
ε           a           b           b
0  ──→1──ε──→2──ε──→3───→4───ε──→5───b──→6───b──→7───b──→8───9(接受)
...
2. **计算 A 的转换**：
   - move(A, a) = {3, 8} → ε-closure({3, 8}) = {1, 2, 3, 4, 6, 7, 8} = B
   - move(A, b) = {5} → ε-closure({5}) = {1, 2, 4, 5, 6, 7} = C

3. **继续计算 B, C 的转换...直到不动点**
...
**目的**：获得状态数最少的 DFA（唯一，不计同构）

#### 可区分状态

如果存在串 x，使得从状态 s、t 出发，一个到达接受状态，一个到达非接受状态，则 x **区分**了 s 和 t。
...
**构造部分**：
```
1. 在 P_final 的每个组中选择一个代表作为最小化 DFA 的状态
2. 开始状态：包含原开始状态的组的代表
3. 接受状态：包含原接受状态的组的代表
4. 转换关系：如果 s 在 a 上的转换到达 t，且 t 所在组的代表为 r，
   则最小化 DFA 中有从 s 到 r 的 a 转换
```
...
**生成流程**：
```
Lex 源程序 (.l)  ──→  Lex 编译器  ──→  lex.yy.c  ──→  C 编译器  ──→  可执行词法分析器
```
...
- **返回值**：将 Token 类型返回给语法分析器

!!! example Lex 文件翻译规则部分

```lex
{ws}           { /* 忽略空白符，不返回任何 Token */ }
