...
![移除non-move-related节点](image-253.png)
...
![计算各个非预着色节点的spill优先级](image-258.png)

从表格中看，`c`的`spill priority`最低，这说明`c`的`degree`很高，但是使用次数很少

![首先spill掉c节点](image-259.png)
![执行合并操作](image-260.png)
![继续执行合并操作](image-261.png)
...
![select](image-263.png)

- 从栈中弹出节点，并为`d`选择颜色`r3`
- 其他所有节点都已经被合并，或者本身就是预着色节点

![rewrite](image-264.png)
![delete](image-265.png)
!!!
